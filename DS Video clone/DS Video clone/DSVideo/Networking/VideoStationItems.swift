...
extension VideoStationWebAPIClient {
    ...
    func getItemDetail(id: String, type: String) async throws -> ItemDetail {
        ...
        if response?.success != true || response?.data?.item == nil {
            ...
            var components2 = URLComponents(url: baseURL.appendingPathComponent("/webapi/entry.cgi"), resolvingAgainstBaseURL: false)!
        ...
        if response?.success != true || response?.data?.item == nil, let idInt = Int(id) {
            ...
            var components3 = URLComponents(url: baseURL.appendingPathComponent("/webapi/entry.cgi"), resolvingAgainstBaseURL: false)!
    ...
    func imageURL(id: String, width: Int? = nil, type: String = "poster", useCacheBusting: Bool = false) -> URL {
        var components = URLComponents(url: baseURL.appendingPathComponent("/webapi/VideoStation/poster.cgi"), resolvingAgainstBaseURL: false)!
        ...
        Self.logger.debug("Poster URL (official format) for id=\(id), type=\(type): \(url.absoluteString)")
        return url
    }
    ...
    func backdropURL(mapperId: String) -> URL {
        ...
        let url = components.url ?? baseURL
        Self.logger.debug("Backdrop URL (official format) for mapper_id=\(mapperId): \(url.absoluteString)")
        return url
    }
    ...
    func items(libraryId: String, limit: Int = 50, offset: Int = 0) async throws -> ItemsResponse {
        ...
        let libs = try await getLibraries()
