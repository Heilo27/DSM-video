...
// MacImageCache (the AppKit twin of ImageCache) lived here — ~39 lines, unreachable
// for the same reason. Removed with the loader body below.
...
struct AuthenticatedImage: View {
  ...
  // The AppKit (NSImage) loader body lived here — ~148 lines. Unreachable: the project
  // has no macOS target, so canImport(AppKit) is false in all four targets. It had
  // already drifted from the live UIKit path above (it cached outside the fetch task
  // rather than inside, so a cancelled task discarded a fully-downloaded image).
}
